...
# -*- coding: utf-8 -*-
"""
Created on Sun Mar  2 19:26:26 2025
...
class DartsTrainerGraph(interface.BaseOneShotTrainer):
    ...
    def _train_one_epoch(self, epoch):
        ...
        for step,(( trn_data), (val_data)) in enumerate(zip(self.train_loader, self.test_loader)):
            ...
            logitsval, lossval = self._logits_and_loss(trn_X, trn_y,train_adj,train_mask)#ON TRAIN DATA NOT VAL
            #metrics = self.metrics(logitsval, val_y)
            metrics = self.metrics(logitsval, trn_y)#name ends with val but actually see it is TRAINING LOSS/METRIC
    ...
    def _logits_and_loss(self, X, y,train_adj,train_mask):
        ...
        return logits, loss
    #my addition
    def _logits_and_lossval(self, X, y,val_adj,val_mask):#ACTUALLY LATER I STOPPED CALLING THIS
        #print('inside logists and loss')
