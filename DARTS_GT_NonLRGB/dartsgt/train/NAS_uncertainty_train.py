...
def extract_optimal_weights_from_metrics(metrics_dict, routing_mode):
    """
    Extract optimal weights for EVERY layer from metrics dict
    """
    try:
        ...
        if 'cumulative_metrics' in metrics_dict and 'alphas' in metrics_dict['cumulative_metrics']:
            ...
            for layer_key, layer_data in cumulative_alphas.items():
                ...
                layer_optimal_weights = []
                for i in range(num_experts):
                    weight = layer_alphas.get(str(i), 0.0)
                    layer_optimal_weights.append(weight)
                
                layer_weights[layer_key] = layer_optimal_weights
                
                logging.info(f"Layer {layer_key} optimal weights: {layer_optimal_weights}")
                for i, (gnn_type, weight) in enumerate(zip(cfg.gt.head_gnn_types, layer_optimal_weights)):
                    logging.info(f"  {gnn_type}: {weight:.4f}")
...
@register_train('nas_uncertainty_train')
def nas_uncertainty_train(loggers, loaders, model, optimizer, scheduler):
    """
