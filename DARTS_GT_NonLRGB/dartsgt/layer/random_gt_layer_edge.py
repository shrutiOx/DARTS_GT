# -*- coding: utf-8 -*-
"""
Created on Fri Jun 27 04:54:22 2025
...
class RANDOMGTLayer(nn.Module):
    """Sequential MPNN + global attention transformer layer with RANDOM GNN selection.
    """
    ...
    def __init__(self, dim_h,
                 ...
                 head_gnn_types=['GINE', 'CustomGatedGCN', 'GATV2']):  # Changed: list of GNN types
        super().__init__()
        ...
        # RANDOM SELECTION: Randomly choose one GNN from the provided list
        self.head_gnn_types = head_gnn_types
        self.attn_gnn = random.choice(head_gnn_types)
        print(f"RANDOMGTLayer: Randomly selected GNN type: {self.attn_gnn}")
        ...
        # Edge Residual Stream (after FF block setup)
        if cfg.dataset.edge_encoder:  # Only if edges exist
            self.edge_processor = nn.Sequential(
                nn.Linear(cfg.gnn.dim_edge, dim_h),
                self.activation(),
                nn.Dropout(dropout),
                nn.Linear(dim_h, dim_h),
                nn.Dropout(dropout)
            )
            # Learnable gate to control edge influence
            self.edge_gate = nn.Parameter(torch.tensor(cfg.gt.edge_startup))
        else:
        ...
        # MPNNs supporting also edge attributes.
        ...
        elif self.attn_gnn == 'CustomGatedGCN':
            ...
            raise ValueError(f"Unsupported kv GNN model: {self.attn_gnn}")
    ...
    def forward(self, batch):
        ...
        if self.self_attn is not None:
           ...
           # q_dense = self.q_projection(q_dense) 
            ...
            # Add Edge Residual Stream
            if self.edge_processor is not None and hasattr(batch, 'edge_attr') and batch.edge_attr is not None:
                # Process edge features
                edge_feat = self.edge_processor(batch.edge_attr)  # [E, dim_h]
                
                # Aggregate to target nodes
                
                edge_aggregated = scatter_add(edge_feat, batch.edge_index[1], 
                                              dim=0, dim_size=h.size(0))  # [N, dim_h]
                
                # Add with learnable gate
                h_attn = h_attn + torch.sigmoid(self.edge_gate) * edge_aggregated
