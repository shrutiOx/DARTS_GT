...
from dartsgt.layer.gps_layer import GPSLayer
...
class GPSModel(torch.nn.Module):
    ...
    def __init__(self, dim_in, dim_out):
        ...
        for i in range(cfg.gt.layers):
            layers.append(GPSLayer(
                ...
                log_attn_weights=cfg.train.mode == 'log-attn-weights'or cfg.gt.pk_explainer.enabled,  # MODIFIED,
            ))
    ...
    def forward(self, batch):
        # Pass attention collector to layers if PK explainer is enabled
        if hasattr(self, 'attention_collector'):
            for layer in self.layers:
                layer.attention_collector = self.attention_collector
        for module in self.children():
            batch = module(batch)
        return batch
