    ...
    def DARTTrain(self, 
        ...
        def dart_accuracy(y_hat, y):
            ...
            if cfg.dataset.task_type == 'regression':
                # Handle both single-task and multi-task regression
                if len(y.shape) > 1 and y.shape[1] > 1:
                    # Multi-task regression: keep 2D shapes
                    mae = torch.nn.L1Loss()(logits, y)
                else:
                    # Single-task regression: may need to squeeze
                    if len(y.shape) == 1 and len(logits.shape) > 1 and logits.shape[1] == 1:
...
def eval_epoch_with_uncertainty(logger, loader, model, split='val'):
    ...
    if split == 'test':
        ...
        if valid_uncertainties:
            ...
            logging.info(f"{split.upper()} Routing Uncertainty: {mean_uncertainty:.4f}Â±{std_uncertainty:.4f}")
...
def nas_uncertainty_train(loggers, loaders, model, optimizer, scheduler):
    ...
    if hasattr(model, 'model'):
        ...
        model.model = discrete_model  # â† Simply replace the inner model
