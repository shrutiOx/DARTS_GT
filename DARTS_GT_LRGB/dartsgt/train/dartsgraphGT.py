...
class DartsTrainerGraphSparse(dartsgraphbase.DartsTrainerGraph):
    ...
    def _train_one_epoch(self, epoch):
        ...
        for step, (trn_batch, val_batch) in enumerate(train_val_iter):
            ...
            # Handle multi-label and multi-task targets differently
            if self.is_multilabel or (task_type == 'regression' and len(trn_batch.y.shape) > 1 and trn_batch.y.shape[1] > 1):
                targets = trn_batch.y  # Keep 2D shape for multi-label and multi-task
                targets_for_metrics = trn_batch.y  # Also keep 2D for metrics
            else:
                targets = trn_batch.y.view(-1)  # Flatten for single-task
            ...
            if task_type == 'regression':
                if len(targets.shape) == 1 and len(logits.shape) > 1 and logits.shape[1] == 1:
        ...
        with torch.no_grad():
            for val_batch in self.test_loader:
                ...
                if isinstance(output, tuple):
                    ...
                    logits = output
                task_type = self._get_task_type()
                if self.is_multilabel or (task_type == 'regression' and len(val_batch.y.shape) > 1 and val_batch.y.shape[1] > 1):
                    targets = val_batch.y  # Keep 2D shape for multi-label and multi-task
                else:
                    targets = val_batch.y.view(-1)  # Flatten for single-task
                # SAME SHAPE HANDLING LOGIC
                if task_type == 'regression':
                    # For single-task regression: ensure same shape
                    if len(targets.shape) == 1 and len(logits.shape) > 1 and logits.shape[1] == 1:
                        logits = logits.squeeze(-1)  # [batch, 1] -> [batch]
                    # For multi-task regression: keep 2D shapes
                    loss = self.loss(logits, targets.float())
                elif task_type in [ 'classification_binary','classification_multilabel']:
                    if self.is_multilabel:
    ...
    def _unrolled_backward(self, trn_batch, val_batch):
        ...
        task_type = self._get_task_type()
        # Handle targets shape based on task
        if self.is_multilabel or (task_type == 'regression' and len(trn_batch.y.shape) > 1 and trn_batch.y.shape[1] > 1):
            loss = self.loss(logits, trn_batch.y.float())
        else:
            loss = self.loss(logits, trn_batch.y.view(-1))
