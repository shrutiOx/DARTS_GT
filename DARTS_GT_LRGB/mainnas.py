...
def main():
    ...
    # Ensure cfg.train exists
    if not hasattr(cfg, 'train'):
        cfg.train = CN()

    # Initialize all 'train' attributes from configpepst.yaml with default values if not present
    if not hasattr(cfg.train, 'mode'):
        cfg.train.mode = 'custom' # Default from configpepst.yaml
    if not hasattr(cfg.train, 'batch_size'):
        cfg.train.batch_size = 32 # Common default, adjust if your project has another specific default
    if not hasattr(cfg.train, 'eval_period'):
        cfg.train.eval_period = 1 # Default from configpepst.yaml
    if not hasattr(cfg.train, 'ckpt_period'):
        cfg.train.ckpt_period = 100 # Default from configpepst.yaml
    if not hasattr(cfg.train, 'auto_resume'):
        cfg.train.auto_resume = False # Common default
    if not hasattr(cfg.train, 'ckpt_clean'):
        cfg.train.ckpt_clean = True # Common default
    if not hasattr(cfg.train, 'epoch_resume'):
        cfg.train.epoch_resume = -1 # Common default
    if not hasattr(cfg.train, 'iter_per_epoch'):
        cfg.train.iter_per_epoch = 32 # Common default
    if not hasattr(cfg.train, 'neighbor_sizes'):
        cfg.train.neighbor_sizes = [20, 15, 10, 5] # Common default
    if not hasattr(cfg.train, 'node_per_graph'):
        cfg.train.node_per_graph = 32 # Common default
    if not hasattr(cfg.train, 'radius'):
        cfg.train.radius = 'extend' # Common default
    if not hasattr(cfg.train, 'sample_node'):
        cfg.train.sample_node = False # Common default
    if not hasattr(cfg.train, 'sampler'):
        cfg.train.sampler = 'full_batch' # Common default
    if not hasattr(cfg.train, 'walk_length'):
        cfg.train.walk_length = 4 # Common default
    
        
    if not hasattr(cfg.gt, 'routing_mode'):
        cfg.gt.routing_mode = 'uniform'  # Default value
    if not hasattr(cfg.gt, 'residual_mult'):
        cfg.gt.residual_mult = 0.1  # Default value
